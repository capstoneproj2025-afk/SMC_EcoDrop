...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('student_id', type=str, help='Student ID to create (e.g., C22-0369)')
        parser.add_argument('--username', type=str, help='Username (defaults to student_id)')
    ...
    def handle(self, *args, **options):
        ...
        student_id = options['student_id'].upper()  # Ensure uppercase
        username = options['username'] or student_id.replace('-', '')  # Remove dash for username
        ...
        try:
            ...
            if User.objects.filter(username=username).exists():
                ...
                if user.profile.student_id != student_id:
                    user.profile.student_id = student_id
                    user.profile.save()
                    self.stdout.write(
                        self.style.SUCCESS(f'Updated student ID for {username}: {student_id}')
                    )
                return
            
            # Check if student ID already exists
            if UserProfile.objects.filter(student_id=student_id).exists():
                existing_profile = UserProfile.objects.get(student_id=student_id)
                self.stdout.write(
                    self.style.ERROR(f'Student ID "{student_id}" already exists for user: {existing_profile.user.username}')
                )
                return
    ...
    def create_sample_users(self):
        ...
        sample_users = [
            {'student_id': 'C22-0369', 'first_name': 'John', 'last_name': 'Doe', 'points': 50},
            {'student_id': 'C23-0145', 'first_name': 'Jane', 'last_name': 'Smith', 'points': 75},
            {'student_id': 'C24-0892', 'first_name': 'Mike', 'last_name': 'Johnson', 'points': 30},
            {'student_id': 'C25-0001', 'first_name': 'Sarah', 'last_name': 'Wilson', 'points': 100},
            {'student_id': 'C21-0555', 'first_name': 'Alex', 'last_name': 'Brown', 'points': 25},
        ]
        ...
        for user_data in sample_users:
            try:
                ...
                username = student_id.replace('-', '').lower()  # c220369, c230145, etc.
                ...
                if UserProfile.objects.filter(student_id=student_id).exists():
                    self.stdout.write(f'Student ID {student_id} already exists, skipping...')
                    continue
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'Created: {student_id} -> {user_data["first_name"]} {user_data["last_name"]} ({username})')
                )
                created_count += 1
                
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'Error creating {user_data["student_id"]}: {str(e)}')
                )
        ...
        for user_data in sample_users:
            self.stdout.write(f'  - {user_data["student_id"]} ({user_data["first_name"]} {user_data["last_name"]})')
        self.stdout.write('')
